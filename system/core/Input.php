...
class Input {
  ...
  private static function fetchFromArray (&$array, $index = null, $xssClean = null) {
    ...
    return (($xssClean === null) ? Config::get ('general', 'global_xss_filtering') : $xssClean) ? Security::xssClean ($value) : $value;
  }
  ...
  public static function requestHeader ($index = null, $xssClean = true) {
    ...
    return (($xssClean === null) ? Config::get ('general', 'global_xss_filtering') : $xssClean) ? Security::xssClean ($headers[$index]) : $headers[$index];
  }
  public static function ip () {
    if (self::$ip !== null) return self::$ip;

    $proxy_ips = Config::get ('general', 'proxy_ips');
  ...
  public static function setCookie ($name, $value = '', $expire = '', $domain = '', $path = '/', $prefix = '', $secure = null, $httponly = null) {
    ...
    if ($prefix === '' && Config::get ('cookie', 'prefix') !== '')
      $prefix = Config::get ('cookie', 'prefix');

    if ($domain == '' && Config::get ('cookie', 'domain') != '')
      $domain = Config::get ('cookie', 'domain');

    if ($path === '/' && Config::get ('cookie', 'path') !== '/')
      $path = Config::get ('cookie', 'path');

    $secure = ($secure === null && Config::get ('cookie', 'secure') !== null) ? (bool) Config::get ('cookie', 'secure') : (bool) $secure;

    $httponly = ($httponly === null && Config::get ('cookie', 'httponly') !== null) ? (bool) Config::get ('cookie', 'httponly') : (bool) $httponly;
