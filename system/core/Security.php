...
class Security {
  ...
  private static function csrfSetHash () {
    if (self::$hash === null) {
      if (isset ($_COOKIE[self::$cookieName]) && is_string ($_COOKIE[self::$cookieName]) && preg_match('#^[0-9a-f]{32}$#iS', $_COOKIE[self::$cookieName]) === 1)
        return self::$hash = $_COOKIE[self::$cookieName];
      $rand = self::getRandomBytes (16);
      self::$hash = ($rand === false) ? md5 (uniqid (mt_rand (), true)) : bin2hex ($rand);
    }
  ...
  public static function csrfVerify () {
    if (strtoupper ($_SERVER['REQUEST_METHOD']) !== 'POST')
      return self::csrfSetCookie ();

    if ($exclude_uris = Config::get ('csrf', 'exclude_uris'))
      foreach ($exclude_uris as $excluded)
        if (preg_match ('#^' . $excluded . '$#i' . (UTF8_ENABLED ? 'u' : ''), URI::uriString ()))
          return ;

    $valid = isset ($_POST[self::$tokenName], $_COOKIE[self::$cookieName]) && hash_equals($_POST[self::$tokenName], $_COOKIE[self::$cookieName]);

    unset ($_POST[self::$tokenName]);

    if (Config::get ('csrf', 'regenerate')) {
    ...
    if ($valid !== true) Exceptions::showError ('您所要求的行為是不允許的。', 403);
